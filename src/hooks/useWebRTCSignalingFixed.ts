...
export const useWebRTCSignaling = ({
  ...
  useEffect(() => {
    ...
    channel
      .on('broadcast', { event: 'offer' }, async (message: SignalingMessage) => {
      ...
      .on('broadcast', { event: 'answer' }, async (message: SignalingMessage) => {
      ...
      .on('broadcast', { event: 'ice-candidate' }, async (message: SignalingMessage) => {
        if (message.from === userId) return;
      ...
      .on('broadcast', { event: 'user-ready' }, (message: SignalingMessage) => {
        console.log('User ready:', message.from, message.role);

        if (role === 'interviewer' && message.role === 'candidate') {
